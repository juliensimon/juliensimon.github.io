#!/usr/bin/env python3
"""
Script to add "About the Author" sections to all missing Arcee blog posts.
This script specifically targets Arcee blog posts that don't have the section.
"""
...
def main():
    """Main function to add About the Author sections to all missing Arcee blog posts."""
    
    print("🎯 Adding 'About the Author' sections to all missing Arcee blog posts...")
    print("📋 This will add sections to Arcee blog posts that don't have them")
    print()
    
    # Find ALL Arcee blog posts
    arcee_files = glob.glob("blog/arcee-posts/**/*.html", recursive=True)
    
    # Remove index.html files
    arcee_files = [f for f in arcee_files if os.path.basename(f) != "index.html"]
    
    print(f"Found files: {arcee_files}")
    
    print(f"📄 Found {len(arcee_files)} Arcee blog posts to process")
    print()
